package com.example.jgi.viewmodel
...
import com.example.jfi.livedata.*
...
class TemplatesViewModel : ViewModel() {
            ...
            override fun onComplete(template: DSTemplateDefinition) {
                ...
                if (template.cacheable.isCacheable) {
                    ...
                    templateDelegate.cacheTemplate(
                        template.templateId,
                        object : DSCacheTemplateListener {
                            override fun onStart() {
                                val cacheTemplateModel =
            ...
            override fun onComplete(template: DSTemplateDefinition?) {
                template?.let {
                    ...
                    templateDelegate.removeCachedTemplate(
                        template,
                        object : DSRemoveTemplateListener {
                            override fun onTemplateRemoved(isRemoved: Boolean) {
                                if (isRemoved) {
                                    val removeCachedTemplateModel = RemoveCachedTemplateModel(
                                        Status.COMPLETE,
                                        template,
                                        position,
                                        null
    ...
    fun retrieveCachedTemplate(templateId: String, position: Int) {
        ...
        templateDelegate.retrieveCachedTemplate(templateId, object : DSGetCachedTemplateListener {
            override fun onComplete(template: DSTemplateDefinition?) {
                if (template != null) {
                    val retrieveCachedTemplateModel =
    ...
    fun useTemplateOnline(
        context: Context,
        templateId: String,
        envelopeDefaults: DSEnvelopeDefaults?
    ) {
